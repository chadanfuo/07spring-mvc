...
<body>
	<form>
		...
		<c:forEach var="eventType" items="${eventTypes }">
			<label> <input type="checkbox" name="types"
				value="${eventType }">${eventType }</label>
		</c:forEach>
	...
	<ul>
		<c:forEach var="event" items="${eventList}">
			<li><a
				href="<%=request.getContextPath() %>/event/detail?id=${event.id}">${event.name }[${event.type}
					}]</a></li>
		</c:forEach>
	</ul>
	추천이벤트:
	<ul>
		<c:forEach var="event" items="${recEventList }">
			<li><a
				href="<%=request.getContextPath()%>/event/detail?id=${event.id}">${event.name }</a></li>
		</c:forEach>
	</ul>
	</form>
</body>
